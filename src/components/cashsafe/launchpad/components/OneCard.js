...
function InfoItem(title, number) {
  return (
    <Grid item xs={4}>
...
export default function OneCard({ data, status, ...other }) {
  const { name, coverUrl, iconUrl, position, collected, softcap, hardcap } = data;
    ...
    return (
      ...
      <Card {...other}>
        <CardMediaStyle>
          ...
          />
          <CoverImgStyle alt="cover" src={`/static/mock-images/covers/${coverUrl}`} />
        </CardMediaStyle>
        ...
        <Grid container sx={{ py: 3, textAlign: 'center' }}>
          {InfoItem('Collected', collected)}
          {InfoItem('Softcap', softcap)}
          {InfoItem('Hardcap', hardcap)}
        </Grid>
      </Card>
      </Grid>
    );
